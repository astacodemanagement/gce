...
<div class="print-element d-none"></div @endsection @push('script') <script src="//cdnjs.cloudflare.com/ajax/libs/numeral.js/2.0.6/numeral.min.js">
</script>
...
<script>
    $(document).ready(function() {
            ...
            ajax: {
                ...
                processResults: function(data) {
                    return {
                        results: $.map(data, function(item) {
                            return {
                                text: item.user.name + ' - ' + item.no_telp, // Menampilkan nama dari tabel users
                                id: item.id,
                                no_telp: item.no_telp,
                                status_cad: item.status_cad,
                                email: item.user.email // Mengambil email dari tabel users
                            };
                        })
                    };
                },
            ...
            ajax: {
                ...
                processResults: function(data) {
                    return {
                        results: $.map(data, function(item) {
                            return {
                                text: item.user.name + ' - ' + item.no_telp, // Menggunakan nama dari tabel users
                                id: item.id,
                                no_telp: item.no_telp,
                                status_cad: item.status_cad,
                                email: item.user.email // Email juga dari tabel users
                            };
                        })
                    };
                },
        ...
        $('#bill_to').select2({
            ...
            ajax: {
                ...
                data: function(params) {
                    return {
                        term: params.term,
                        selectedKonsumen: {
                            konsumen: $('#konsumen').val(), // Kirim nilai konsumen yang dipilih
                            konsumenPenerima: $('#konsumen_penerima').val() // Kirim nilai konsumen penerima yang dipilih
                        }
                    };
                },
                processResults: function(data) {
                    return {
                        results: $.map(data, function(item) {
                            return {
                                text: item.user.name + ' - ' + item.no_telp, // Gunakan nama dari tabel users
                                id: item.id,
                                no_telp: item.no_telp,
                                email: item.user.email // Email dari tabel users
                            };
                        })
                    };
                },
            ...
            var data = e.params.data;
            // Tambahkan logika Anda di sini untuk menangani peristiwa select2:select
        });
